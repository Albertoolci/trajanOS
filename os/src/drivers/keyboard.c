    ...
    if (scancode < sizeof(scancode_ascii_es)) {
        ...
        if (ascii_char) {
            if (ascii_char == '\b') {
                if (buffer_index > 0) {
                    buffer_index--;
                    key_buffer[buffer_index] = '\0'; // Null-terminate the buffer after backspace
                    backspace();
                }

            } else if (ascii_char == '\n') {
                key_buffer[buffer_index] = '\0'; // Null-terminate the buffer on Enter
                kprintf("\n");
                shell_parse(key_buffer);
                buffer_index = 0; // Reset buffer index for the next line

            } else if (buffer_index < KEY_BUFFER_SIZE - 1) {
                key_buffer[buffer_index++] = ascii_char;
